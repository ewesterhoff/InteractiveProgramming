...
class Banana(pygame.sprite.Sprite):
    def __init__(self, length, speed, position, screen, color):
        # Call the parent class (Sprite) constructor
        pygame.sprite.Sprite.__init__(self)
        self.length = length
        self.height = .3*length
        self.speed = speed
        self.x, self.y = position
        self.screen = screen
        self.color = color

        #self.image = pygame.Surface([length, self.height])
    ...
    def draw(self):
        # draw circle on Sprite surface
        self.screen.blit(self.image, (self.x, self.y))
        #pygame.draw.circle(self.screen, self.color, (self.x, self.y), self.length)
